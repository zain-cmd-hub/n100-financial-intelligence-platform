...
from src.etl.validator import (
    check_primary_key, check_composite_key, check_foreign_key,
    check_balance_sheet, check_opm, check_positive_sales,
    check_net_cash_flow, check_tax_rate, check_dividend_payout,
    check_eps_sign, check_annual_report_url, check_bse_profile,
    check_roe_range, check_company_website
)
...
def test_check_composite_key():
    df = pd.DataFrame({"company_id": ["A", "A"], "year": [2020, 2020], "company_name": ["A", "A"]})
...
def test_check_balance_sheet():
    df = pd.DataFrame({"id": [1], "company_name": ["A"], "company_id": ["A"], "year": [2020], "total_liabilities": [100], "total_assets": [90]})
    fails = get_failures(check_balance_sheet, df)
    assert len(fails) == 1
    assert fails[0][1] == "WARNING"
    
def test_check_opm():
    df = pd.DataFrame({"company_id": ["A"], "year": [2020], "operating_profit": [10], "sales": [100], "opm_percentage": [20.0]})
...
def test_check_eps_sign():
    df = pd.DataFrame({"company_id": ["A"], "year": [2020], "net_profit": [-10], "eps": [5]})
    fails = get_failures(check_eps_sign, df)
    assert len(fails) == 1
    assert fails[0][1] == "WARNING"

def test_check_annual_report_url():
    df = pd.DataFrame({"id": [1], "company_name": ["A"], "Annual_Report": ["http://invalid"], "is_url_valid": [False]})
    fails = get_failures(check_annual_report_url, df)
    assert len(fails) >= 0

def test_check_bse_profile():
    df = pd.DataFrame({"id": [1], "company_name": ["A"], "bse_profile": ["invalid_url"]})
