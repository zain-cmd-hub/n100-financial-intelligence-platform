...
num_cols = ['sales', 'return_on_equity_pct', 'market_cap_crore', 'pe_ratio', 'net_profit_margin_pct']
for c in num_cols:
    if c in df.columns:
...
sector_df.dropna(subset=['sales', 'return_on_equity_pct', 'market_cap_crore'], inplace=True)
...
fig_bubble = px.scatter(
    ...
    hover_data={'sales': ':.2f', 'return_on_equity_pct': ':.2f', 'market_cap_crore': ':.2f'},
    labels={
        "sales": "Revenue (Cr)",
...
fig_bubble.update_layout(
    xaxis=dict(showgrid=True, zeroline=True, zerolinecolor='lightgrey'),
    yaxis=dict(showgrid=True, zeroline=True, zerolinecolor='lightgrey'),
    hovermode='closest'
)
...
kpi_df = sector_df.groupby('sub_sector')[['pe_ratio', 'return_on_equity_pct', 'net_profit_margin_pct']].median().reset_index()

# Reshape for grouped bar chart
kpi_melted = kpi_df.melt(id_vars='sub_sector', var_name='Metric', value_name='Median Value')
rename_metrics = {
    'pe_ratio': 'Median P/E',
...
with st.expander("Show Raw Data"):
    st.dataframe(sector_df[['company_id', 'company_name_company', 'sub_sector', 'sales', 'return_on_equity_pct', 'market_cap_crore', 'pe_ratio']], hide_index=True, use_container_width=True)
