...
from src.dashboard.utils.db import get_companies, get_sectors, get_pl, get_ratios, get_prosandcons, get_bs
...
with col_card2:
    ...
    st.caption(f"**Ticker:** {ticker} | **Sector:** {sector_data['broad_sector'].iloc[0] if not sector_data.empty else 'N/A'} | **Sub-Sector:** {sector_data['sub_sector'].iloc[0] if not sector_data.empty else 'N/A'}")
...
if not pl.empty: pl['year'] = pl['year'].astype(str)
if not ratios.empty: ratios['year'] = ratios['year'].astype(str)
if not bs.empty: bs['year'] = bs['year'].astype(str)

latest_year = ratios['year'].max() if not ratios.empty else None
latest_ratios = ratios[ratios['year'] == latest_year].iloc[0] if latest_year and not ratios.empty else None
...
with col_chart1:
    ...
    if not pl.empty:
        ...
        pl_melted = pl_chart.melt(id_vars=['year'], value_vars=['sales', 'net_profit'], var_name='Metric', value_name='Cr')
        pl_melted['Metric'] = pl_melted['Metric'].map({'sales': 'Revenue', 'net_profit': 'Net Profit'})
        fig1 = px.bar(pl_melted, x='year', y='Cr', color='Metric', barmode='group', title="Revenue & Net Profit (Cr)")
...
with col_chart2:
    ...
    if not ratios.empty and not pl.empty and not bs.empty:
        merged = ratios[['year', 'return_on_equity_pct']].merge(pl[['year', 'operating_profit']], on='year')
        merged = merged.merge(bs[['year', 'equity_capital', 'reserves', 'borrowings']], on='year')
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=merged['year'], y=merged['ROE'], name='ROE', mode='lines+markers', line=dict(color='blue')))
        fig2.add_trace(go.Scatter(x=merged['year'], y=merged['ROCE'], name='ROCE', mode='lines+markers', line=dict(color='orange'), yaxis='y2'))
        
        fig2.update_layout(
            title="ROE & ROCE over Time",
            yaxis=dict(title="ROE (%)", titlefont=dict(color="blue"), tickfont=dict(color="blue")),
            yaxis2=dict(title="ROCE (%)", titlefont=dict(color="orange"), tickfont=dict(color="orange"), anchor="x", overlaying="y", side="right"),
            hovermode="x unified"
        )
