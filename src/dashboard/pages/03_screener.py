...
fcf = st.sidebar.slider("FCF Min (Cr)", min_value=-10000.0, max_value=50000.0, key='fcf_min')
rev_cagr = st.sidebar.slider("Rev CAGR 5y Min (%)", min_value=-50.0, max_value=100.0, key='rev_cagr_min')
pat_cagr = st.sidebar.slider("PAT CAGR 5y Min (%)", min_value=-50.0, max_value=150.0, key='pat_cagr_min')
...
div_yield = st.sidebar.slider("Div Yield Min (%)", min_value=0.0, max_value=10.0, key='div_yield_min')
...
numeric_cols = ['return_on_equity_pct', 'debt_to_equity', 'free_cash_flow_cr', 'revenue_cagr', 
                'pat_cagr', 'operating_profit_margin_pct', 'pe_ratio', 'pb_ratio', 
                'dividend_yield_pct', 'interest_coverage']

for c in numeric_cols:
    if c in filtered_df.columns:
        filtered_df[c] = pd.to_numeric(filtered_df[c].replace("Debt Free", 99999), errors='coerce')

# Apply filters safely (if column exists)
if 'return_on_equity_pct' in filtered_df: filtered_df = filtered_df[filtered_df['return_on_equity_pct'] >= roe]
if 'debt_to_equity' in filtered_df: filtered_df = filtered_df[filtered_df['debt_to_equity'] <= de]
if 'free_cash_flow_cr' in filtered_df: filtered_df = filtered_df[filtered_df['free_cash_flow_cr'] >= fcf]
if 'revenue_cagr' in filtered_df: filtered_df = filtered_df[filtered_df['revenue_cagr'] >= rev_cagr]
if 'pat_cagr' in filtered_df: filtered_df = filtered_df[filtered_df['pat_cagr'] >= pat_cagr]
if 'operating_profit_margin_pct' in filtered_df: filtered_df = filtered_df[filtered_df['operating_profit_margin_pct'] >= opm]
if 'pe_ratio' in filtered_df: filtered_df = filtered_df[filtered_df['pe_ratio'] <= pe]
if 'pb_ratio' in filtered_df: filtered_df = filtered_df[filtered_df['pb_ratio'] <= pb]
if 'dividend_yield_pct' in filtered_df: filtered_df = filtered_df[filtered_df['dividend_yield_pct'] >= div_yield]
if 'interest_coverage' in filtered_df: filtered_df = filtered_df[filtered_df['interest_coverage'] >= icr]

# Format output table
display_cols = ['company_id', 'company_name_company', 'sector_name', 'composite_score'] + numeric_cols
