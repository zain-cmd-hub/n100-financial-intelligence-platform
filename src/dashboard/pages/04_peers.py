...
from src.dashboard.utils.db import get_peer_group_names, get_peers, get_screener_data, get_companies
...
default_benchmark = peers_df[peers_df['is_benchmark'] == 1]['company_id'].iloc[0] if 1 in peers_df['is_benchmark'].values else peer_tickers[0]

benchmark_company = st.selectbox("Select Benchmark Company", options=peer_tickers, index=peer_tickers.index(default_benchmark))
...
group_data = group_data.merge(group_companies[['id', 'roce_percentage', 'company_name']], left_on='company_id', right_on='id', how='left')

# 2. Side-by-side KPI Table
st.subheader(f"KPI Comparison: {selected_group}")
table_cols = ['company_id', 'company_name', 'return_on_equity_pct', 'roce_percentage', 'pe_ratio', 'pb_ratio', 'revenue_cagr']
...
def highlight_benchmark(row):
    color = 'background-color: rgba(255, 215, 0, 0.2)' if row['Ticker'] == benchmark_company else ''
    return [color] * len(row)

styled_df = compare_df.style.apply(highlight_benchmark, axis=1)
# Highlight best metrics (Max for returns, Min for valuation)
styled_df = styled_df.highlight_max(subset=['ROE (%)', 'ROCE (%)', 'Rev CAGR (%)'], color='rgba(0, 255, 0, 0.2)')
...
radar_metrics = ['return_on_equity_pct', 'roce_percentage', 'operating_profit_margin_pct', 
                 'net_profit_margin_pct', 'revenue_cagr', 'pat_cagr', 'pe_ratio', 'pb_ratio']
radar_labels = ['ROE', 'ROCE', 'OPM', 'NPM', 'Rev CAGR', 'PAT CAGR', 'P/E (Inv)', 'P/B (Inv)']
...
bench_data = radar_df[radar_df['company_id'] == benchmark_company][radar_metrics].iloc[0]
...
st.caption("Note: Metrics are normalized to a 0-100 scale for visualization. For Valuation metrics (P/E, P/B), a higher score indicates a more attractive (lower) valuation.")
