...
selected_year = st.sidebar.selectbox(
    "Select Year", options=[2024, 2023, 2022, 2021, 2020, 2019]
)
...
if not ratios_yr.empty:
    ...
    median_rev_cagr = (
        ratios_yr["revenue_cagr"].median()
        if "revenue_cagr" in ratios_yr.columns
        else None
    )
else:
    avg_roe = median_pe = median_de = total_companies = median_rev_cagr = (
        debt_free_count
    ) = None
...
col4.metric(
    "Total Companies", f"{total_companies}" if pd.notna(total_companies) else "0"
)
col5.metric(
    "Median Rev CAGR (5y)",
    f"{median_rev_cagr:.2f}%" if pd.notna(median_rev_cagr) else "N/A",
)
col6.metric(
    "Debt-Free Companies", f"{debt_free_count}" if pd.notna(debt_free_count) else "0"
)
