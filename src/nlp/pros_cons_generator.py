...
def evaluate_rules():
    """Handles operations for evaluate_rules."""
    ...
    for _, comp_row in df_screener.iterrows():
        ...
        def check_trend(df, col, trend="up", n_years=3):
            """Handles operations for check_trend."""
            if df.empty or len(df) < n_years:
                return 0, False
            recent = df.sort_values("year", ascending=False).head(n_years)
            vals = (
                pd.to_numeric(recent[col].replace("Debt Free", "0"), errors="coerce")
                .fillna(0)
                .values
            )
            if len(vals) < n_years:
        ...
        conf, passed = check_sustained(
            ratios, "return_on_equity_pct", lambda x: x > 20, 3
        )
        ...
        if len(bs) >= 2:
            ...
            assets = (
                pd.to_numeric(bs_recent["total_assets"], errors="coerce")
                .fillna(0)
                .values
            )
            debts = (
                ...
                results.append(
                    {
                        ...
                        "type": "con",
                        "rule_id": "C1",
                        "text": f"Debt-to-equity ratio of {de:.1f}x is elevated for a non-financial company and warrants monitoring",
                        "confidence_pct": conf,
                    }
                )
                con_count += 1
            ...
            results.append(
                {
                    ...
                    "type": "pro",
                    "rule_id": "P_FALLBACK",
                    "text": "Stable market position within the Nifty 100 universe",
                    "confidence_pct": 100.0,
                }
            )
        if con_count == 0:
            results.append(
                {
                    ...
                    "type": "con",
                    "rule_id": "C_FALLBACK",
                    "text": "Macro-economic sensitivity and broad sector cyclicality",
                    "confidence_pct": 100.0,
                }
            )
    ...
    assert (
        pro_cids == expected
    ), f"Exit Criteria failed: {pro_cids} companies have pros, expected {expected}"
    assert (
        con_cids == expected
    ), f"Exit Criteria failed: {con_cids} companies have cons, expected {expected}"
    logger.info(
        "Exit Criteria successfully met: Every company has at least 1 pro and 1 con."
    )
