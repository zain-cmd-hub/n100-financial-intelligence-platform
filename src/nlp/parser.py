...
def parse_analysis_text():
    ...
    if not df_failures.empty:
        ...
        # Create empty file
        pd.DataFrame(columns=['company_id', 'metric_type', 'raw_text']).to_csv(failures_path, index=False)
        logger.info(f"No parsing failures detected.")
    ...
    if not df_parsed.empty:
        ...
        if divergences:
            ...
            logger.warning(f"Found {len(df_div)} divergence flags > 5%. Saved to {div_path}")
        else:
            pd.DataFrame(columns=['company_id', 'metric_type', 'parsed_value', 'computed_value', 'difference']).to_csv(div_path, index=False)
