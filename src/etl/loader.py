...
def align_dataframe_to_table(connection, df, table_name):
    """
    ...
    matching_columns = [column for column in table_columns if column in source_columns]

    if not matching_columns:

        raise ValueError(f"No matching columns found " f"for table: {table_name}")
...
def remove_duplicate_primary_keys(df, table_name):
    """
    ...
    duplicate_mask = df.duplicated(subset=["id"], keep="first")

    duplicate_count = int(duplicate_mask.sum())

    if duplicate_count > 0:

        print(f"  ⚠ Duplicate primary keys removed: " f"{duplicate_count}")
...
def filter_invalid_foreign_keys(connection, df, table_name):
    """
    ...
    valid_company_ids = set(
        valid_company_ids.dropna().astype(str).str.strip().str.upper()
    )
    ...
    if rejected_count > 0:
        ...
        invalid_ids = sorted(company_ids[invalid_mask].unique())

        print(f"  ⚠ FK-invalid rows rejected: " f"{rejected_count}")

        print(f"  Invalid company IDs: " f"{invalid_ids}")
...
def initialize_database():
    ...
    if not SCHEMA_FILE.exists():
        ...
        raise FileNotFoundError(f"Schema file not found: " f"{SCHEMA_FILE}")
...
def run_full_data_load():
    ...
    try:
        ...
        for dataset in DATASETS:
            ...
            print(f"Loading: {filename} " f"→ {table_name}")
            ...
            try:
                ...
                df = load_excel(filename, header)

                source_rows = len(df)

                print(f"  Source rows: " f"{source_rows}")

                # =======================================
                # ALIGN WITH DATABASE SCHEMA
                # =======================================

                df = align_dataframe_to_table(connection, df, table_name)

                # =======================================
                # REMOVE DUPLICATE PRIMARY KEYS
                # =======================================

                df, duplicate_rows = remove_duplicate_primary_keys(df, table_name)

                # =======================================
                # FOREIGN KEY VALIDATION
                # =======================================

                df, fk_rejected = filter_invalid_foreign_keys(
                    connection, df, table_name
                )
                ...
                loaded_rows = insert_dataframe(connection, df, table_name)

                connection.commit()

                print(f"  ✅ Loaded rows: " f"{loaded_rows}")

                print(f"  Rejected rows: " f"{rejected_rows}")
        ...
        if not foreign_key_errors:
            ...
            print("✅ PRAGMA foreign_key_check " "returned 0 rows.")

        else:

            print(f"❌ Foreign key violations found: " f"{len(foreign_key_errors)}")

            for error in foreign_key_errors[:10]:
        ...
        print(f"Tables audited: " f"{len(audit_df)}")
