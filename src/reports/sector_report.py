...
def generate_sector_reports():
    ...
    style_header = ParagraphStyle(
        'Header', parent=styles['Heading1'], fontSize=18, textColor=colors.white,
        backColor=colors.HexColor('#1f497d'), alignment=1, spaceAfter=20, borderPadding=10
    )
    ...
    for sector in sectors:
        ...
        doc = SimpleDocTemplate(str(pdf_path), pagesize=landscape(A4), rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=30)
        ...
        med_mcap = sector_df['market_cap'].median() if 'market_cap' in sector_df.columns else 0
        med_pe = sector_df['pe'].median() if 'pe' in sector_df.columns else 0
        med_roce = sector_df['roce_pct'].median() if 'roce_pct' in sector_df.columns else 0
        med_roe = sector_df['roe_pct'].median() if 'roe_pct' in sector_df.columns else 0
        med_fcf = sector_df['free_cash_flow_cagr'].median() if 'free_cash_flow_cagr' in sector_df.columns else 0
        
        row1 = [
            Paragraph(f"<b>Median Market Cap</b><br/>₹ {med_mcap:,.0f} Cr", style_normal),
        ...
        row2 = [
            ...
            Paragraph(f"<b>-</b><br/>-", style_normal)
        ]
        ...
        for _, row in sector_df.iterrows():
            ...
            table_data.append([
                ...
                Paragraph(c_name, style_normal), # Wordwrap will happen here natively
        ...
        roster_table = Table(table_data, colWidths=[1*inch, 2.5*inch, 1*inch, 1*inch, 0.8*inch, 0.8*inch, 0.8*inch, 1*inch])
