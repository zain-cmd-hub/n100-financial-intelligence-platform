...
CF_INTEL_PATH = (
    Path(__file__).resolve().parents[2] / "output" / "cashflow_intelligence.xlsx"
)


def get_company_name(ticker):
    """Handles operations for get_company_name."""
...
def get_kpis(ticker):
    """Handles operations for get_kpis."""
    df = get_screener_data()
    row = df[df["company_id"] == ticker]
    if row.empty:
        return {}
...
def get_nlp_points(ticker):
    """Handles operations for get_nlp_points."""
    pros = []
    cons = []
...
def get_capital_allocation(ticker):
    """Handles operations for get_capital_allocation."""
    if CF_INTEL_PATH.exists():
        cf_intel = pd.read_excel(CF_INTEL_PATH)
        intel_row = cf_intel[cf_intel["company_id"] == ticker]
        if not intel_row.empty:
...
def chart_rev_pat(ticker):
    """Handles operations for chart_rev_pat."""
    pl = get_pl(ticker)
    if pl.empty:
        return None
...
def chart_roe_roce(ticker):
    """Handles operations for chart_roe_roce."""
    ...
    if "return_on_capital_employed_pct" not in ratios.columns:
        ratios["return_on_capital_employed_pct"] = 0

    roe = pd.to_numeric(ratios["return_on_equity_pct"], errors="coerce").fillna(0)
    roce = pd.to_numeric(
        ratios["return_on_capital_employed_pct"], errors="coerce"
    ).fillna(0)
...
def chart_bs_composition(ticker):
    """Handles operations for chart_bs_composition."""
    bs = get_bs(ticker)
    if bs.empty:
        return None
    ...
    if "equity_share_capital" not in bs.columns:
        bs["equity_share_capital"] = 0
    if "reserves" not in bs.columns:
        bs["reserves"] = 0
    if "borrowings" not in bs.columns:
        bs["borrowings"] = 0
    if "other_liabilities" not in bs.columns:
        bs["other_liabilities"] = 0

    eq = pd.to_numeric(bs["equity_share_capital"], errors="coerce").fillna(
        0
...
def chart_cf_waterfall(ticker):
    """Handles operations for chart_cf_waterfall."""
    cf = get_cf(ticker)
    if cf.empty:
        return None
...
def build_tearsheet(ticker):
    """Handles operations for build_tearsheet."""
    ...
    style_pro = ParagraphStyle(
        "Pro", parent=style_normal, spaceAfter=5, textColor=colors.black
    )
    style_con = ParagraphStyle(
        "Con", parent=style_normal, spaceAfter=5, textColor=colors.black
    )
    ...
    story.append(
        Paragraph(f"<b>{comp_name} ({ticker})</b> - Financial Tearsheet", style_header)
    )
    ...
    img_rev = (
        Image(buf_rev, width=3.2 * inch, height=1.6 * inch)
        if buf_rev
        else Paragraph("No Data", style_normal)
    )
    img_roe = (
        Image(buf_roe, width=3.2 * inch, height=1.6 * inch)
        if buf_roe
        else Paragraph("No Data", style_normal)
    )
    ...
    img_bs = (
        Image(buf_bs, width=3.2 * inch, height=1.6 * inch)
        if buf_bs
        else Paragraph("No Data", style_normal)
    )
    img_cf = (
        Image(buf_cf, width=3.2 * inch, height=1.6 * inch)
        if buf_cf
        else Paragraph("No Data", style_normal)
    )
...
def generate_all():
    """Handles operations for generate_all."""
    df = get_screener_data()
    ...
    # Log skipped
    skip_path = (
        Path(__file__).resolve().parents[2] / "output" / "skipped_tearsheets.csv"
    )
    pd.DataFrame({"company_id": skipped}).to_csv(skip_path, index=False)
    logger.info(
        f"Batch generation complete. Generated: {len(all_tickers) - len(skipped)}, Skipped: {len(skipped)}"
    )
