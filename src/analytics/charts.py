...
def create_radar_chart(
    company_id, company_name, peer_group, company_values, peer_averages, labels
):
    """
    ...
    plt.yticks(
        [0.25, 0.5, 0.75, 1.0], ["25th", "50th", "75th", "100th"], color="grey", size=8
    )
    plt.ylim(0, 1.0)
...
def generate_charts():
    """Handles operations for generate_charts."""
    ...
    for col in AXES:
        if col not in df.columns:
            ...
            if df[col].dtype == object and col == "debt_to_equity":
    ...
    # Calculate Ranks
    for col in AXES:
        rank = df.groupby(["peer_group_name", "year"])[col].rank(
        ...
        global_rank = df.groupby(["year"])[col].rank(
            pct=True, ascending=True, na_option="bottom"
        )
        if col == "debt_to_equity":
    ...
    avg_cols = [f"{col}_pct" for col in AXES]
    ...
    for _, row in latest_df.iterrows():
        ...
        comp_vals = [row.get(f"{col}_pct", 0.0) for col in AXES]
        comp_vals = [0.0 if pd.isna(v) else v for v in comp_vals]

        if pd.isna(peer_group):
            peer_group = None
            avg_vals = [nifty_avg.get(f"{col}_pct", 0.5) for col in AXES]
        else:
            p_dict = peer_averages.get(peer_group, {})
            avg_vals = [p_dict.get(f"{col}_pct", 0.5) for col in AXES]
