...
def generate_report():
    ...
    if cf_intel_path.exists():
        ...
        # Join latest pattern
        latest_mapping = latest_alloc[['company_id', 'pattern']].rename(columns={'pattern': 'capital_allocation_label'})
        df_intel = pd.merge(df_intel, latest_mapping, on='company_id', how='left')
        
        df_intel.to_excel(cf_intel_path, index=False)
        logger.info(f"Updated {cf_intel_path.name} with true capital allocation labels.")
    else:
    ...
    for cid in valid_companies:
        comp_data = df_alloc[df_alloc['company_id'] == cid].sort_values('year', ascending=True)
        if len(comp_data) >= 2:
    ...
    logger.info(f"Identified {len(df_changes)} YoY pattern changes. Saved to pattern_changes.csv.")
