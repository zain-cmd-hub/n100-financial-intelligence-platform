...
def generate_report():
    ...
    df = df.merge(
        peers_df[["company_id", "peer_group_name", "is_benchmark"]].drop_duplicates(),
        on="company_id",
        how="left",
    )
    ...
    # Formats
    format_pct_green = workbook.add_format(
        {"bg_color": "#C6EFCE", "font_color": "#006100", "num_format": "0.00"}
    )
    format_pct_yellow = workbook.add_format(
        {"bg_color": "#FFEB9C", "font_color": "#9C5700", "num_format": "0.00"}
    )
    format_pct_red = workbook.add_format(
        {"bg_color": "#FFC7CE", "font_color": "#9C0006", "num_format": "0.00"}
    )
    format_benchmark = workbook.add_format({"bg_color": "#FFC000", "bold": True})
    format_num = workbook.add_format({"num_format": "0.00"})
    format_median = workbook.add_format(
        {"bold": True, "bg_color": "#D3D3D3", "num_format": "0.00"}
    )
