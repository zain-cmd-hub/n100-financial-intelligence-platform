...
from src.dashboard.utils.db import get_screener_data, get_ratios, get_pl, get_bs, get_cf
...
def compute_cashflow_kpis():
    ...
    for _, row in df_screener.iterrows():
        ...
        cfo_pat = pd.merge(cf_recent[['year', 'operating_activity']], pl_recent[['year', 'net_profit']], on='year', how='inner')
        
        cfo_vals = pd.to_numeric(cfo_pat['operating_activity'], errors='coerce').fillna(0)
        ...
        inv_act = pd.to_numeric(latest_cf['investing_activity'].values[0], errors='coerce') if not latest_cf.empty else 0
        sales = pd.to_numeric(latest_pl['sales'].values[0], errors='coerce') if not latest_pl.empty else 0
        ...
        cfo_latest = pd.to_numeric(latest_cf['operating_activity'].values[0], errors='coerce') if not latest_cf.empty else 0
        cff_latest = pd.to_numeric(latest_cf['financing_activity'].values[0], errors='coerce') if not latest_cf.empty else 0
        net_profit_latest = pd.to_numeric(latest_pl['net_profit'].values[0], errors='coerce') if not latest_pl.empty else 0
        ...
        borrowings = pd.to_numeric(bs_recent['borrowings'], errors='coerce').fillna(0).values
        ...
        de_latest = pd.to_numeric(str(row.get('debt_to_equity')).replace('Debt Free', '0'), errors='coerce')
        ...
        fcf_latest = pd.to_numeric(latest_ratios['free_cash_flow_cr'].values[0], errors='coerce') if not latest_ratios.empty else 0
    ...
    if not df_alerts.empty:
        ...
        pd.DataFrame(columns=['company_id', 'CFO_value', 'CFF_value', 'latest_net_profit']).to_csv(out_csv, index=False)
